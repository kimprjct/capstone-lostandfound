...
@section('title', 'Manage Found Item')
@section('page-title', 'Manage Found Item')

@section('content')
<div class="mb-6">
    <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold">Manage Found Item</h3>
        <a href="{{ route('tenant.found-items.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-md text-sm font-medium">
            <i class="fas fa-arrow-left mr-1"></i> Back to Found Items
        </a>
    </div>
    ...
    <!-- Item Details -->
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    @if($foundItem->image_url)
                        <div class="relative">
                        <img src="{{ $foundItem->image_url }}" alt="{{ $foundItem->title }}" 
            ...
            <div class="lg:col-span-2">
                    ...
                    <div class="bg-blue-50 rounded-lg p-4">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <p class="mt-1 text-sm font-medium text-gray-900">{{ $foundItem->title }}</p>
                            </div>
                <div>
                                <label class="block text-xs font-medium text-blue-700 uppercase tracking-wide">Category</label>
                    <p class="mt-1 text-sm text-gray-900">{{ $foundItem->category ?? 'N/A' }}</p>
                </div>
                <div>
                                <label class="block text-xs font-medium text-blue-700 uppercase tracking-wide">Status</label>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                        @if($foundItem->status === 'unclaimed') bg-blue-100 text-blue-800
                        @elseif($foundItem->status === 'under_review') bg-yellow-100 text-yellow-800
                        @elseif($foundItem->status === 'claimed') bg-green-100 text-green-800
                        @elseif($foundItem->status === 'cancelled') bg-gray-100 text-gray-800
                ...
                <div class="space-y-3">
                            <div>
                                <label class="block text-xs font-medium text-green-700 uppercase tracking-wide">Location Found</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $foundItem->location ?? 'N/A' }}</p>
                                    </div>
                            <div>
                                <label class="block text-xs font-medium text-green-700 uppercase tracking-wide">Date Found</label>
                                <p class="mt-1 text-sm text-gray-900">
                                    {{ $foundItem->date_found ? $foundItem->date_found->format('F d, Y') : 'N/A' }}
                                </p>
                                </div>
                            <div>
                                <label class="block text-xs font-medium text-green-700 uppercase tracking-wide">Time Found</label>
                                <p class="mt-1 text-sm text-gray-900">
                                    {{ $foundItem->time_found ? \Carbon\Carbon::createFromFormat('H:i:s', $foundItem->time_found)->format('g:i A') : 'N/A' }}
                                </p>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-green-700 uppercase tracking-wide">Found By</label>
                                <p class="mt-1 text-sm text-gray-900">
